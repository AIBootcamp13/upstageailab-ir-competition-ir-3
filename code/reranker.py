...
# 5. reranker
"""
def rerank(question:str) -> result: List[str] :
    rerank_lst = []
    docs: List[Document] = retriever.invoke(question)
    for doc in docs:
        rerank = {}
        score_similar = reranker.score(doc.page_content, question)
        rerank["score"] = score_simialr
        rerank["doc"]
        rerank_lst.append(rerank)
...
if __name__ == "__main__":
    ...
    output_path = "/data/ephemeral/home/dev/IR/data/eval_with_rerank_submissuion_3_mod.jsonl"
    with (
